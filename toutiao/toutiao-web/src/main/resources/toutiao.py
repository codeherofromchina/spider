...
conn.hset("headerKey","Referer",'https://www.toutiao.com/ch/news_image/')
...
while True:
	##获取签名并发送信息
	response= pub.parse_response()
	print('listener response:' + str(response))
	max_behot_time = response[2].decode()
	sinature = firefox.execute_script('return TAC.sign(' + max_behot_time + ')')
